...
const { downloadFromFTP } = require("./ftpClient");
...
async function syncFiles(fileGroup) {
  ...
  try {
    ...
    console.log(
      `✅ Sync task for ${fileGroup} completed successfully in ${duration} seconds.`
    );
  } catch (error) {
    ...
    for (const localFilePath of localFilePaths) {
      try {
        ...
        console.log(
          `ℹ️ Local copy already deleted or does not exist: ${localFilePath}`
        );
      }
    }
  }
...
function syncRegularFiles() {
  console.log(
    "⏰ Cron task for REGULAR files triggered at:",
...
function syncNightlyFiles() {
  console.log(
    "⏰ Cron task for NIGHTLY files triggered at:",
    new Date().toLocaleString()
  );
